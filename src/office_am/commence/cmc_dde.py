...
from .dde import get_all_connected, get_conversation_func, get_data, get_record


def get_fields(
        conv, table: str, item: str, fields: List[str] = None, delim=';'
) -> Dict[str, str]:
    if fields is None:
        fields = ['Name']
...
def get_connected_item_names(
        conv, from_category: str, from_item: str, connection: str, to_category: str
) -> List[str]:
...
def get_field_to_file(
        conv, category: str, item: str, field: str, filename: str
) -> None:
...
def get_commence_data(
        table, name, fields: Iterable[str], connections: Iterable[Connector] = None
):
    results = {}
    ...
    for connection in connections:
        connected_data = get_all_connected(conv, table, connection)
        results[connection.key_table] = connected_data
    return results
